...
const dicionarioDeSinonimos = {
  ...
  // AIR FORCE
  'air force': 'Air Force', 'air': 'Air Force', 'ar force': 'Air Force', 'ai foice': 'Air Force', 'air foise': 'Air Force', 'air foisse': 'Air Force', 'ar foice': 'Air Force',
  // AIR FORCE STUSSY
  ...
  // DUNK
  'dunk': 'Dunk', 'duk': 'Dunk', 'dank': 'Dunk', 'dunke': 'Dunk', 'danke': 'Dunk', 'dunk low': 'Dunk', 'dunke low': 'Dunk', 'danke low': 'Dunk', 'dank low': 'Dunk', 'dunk lou': 'Dunk',
  // DUNK TWIST
  ...
  'force': 'Air Force', 'foice': 'Air Force', 'forci': 'Air Force',
  // Atributos de Tênis
...
function isDescendenteDe(categoriaId, categoriaPaiId, todasAsCategoriasMap) {
  ...
  let visitados = new Set(); // Para evitar loops infinitos

  while (categoriaAtual) {
    if (visitados.has(categoriaAtual.id)) return false; // Ciclo detectado
...
export default async function handler(request, response) {
  ...
  try {
    const nuvemShopResponse = await fetch('https://api.nuvemshop.com.br/v1/905119/categories', {
      headers: {
        'Authentication': 'bearer 972ade7aae8a494d58d98d89e2730b2067d01d4a',
    ...
    // Otimização: Crie um Map para buscas rápidas de categorias por ID
    ...
    const categoriaNacional = candidatos.find(c => isDescendenteDe(c.id, ID_CATEGORIA_NACIONAL, categoriasMap));
    const categoriaPremium = candidatos.find(c => isDescendenteDe(c.id, ID_CATEGORIA_PREMIUM, categoriasMap));
